...
*/
class Solution_3421_이승규 {
	static int n, m, answer;
	static int[] badIngA, badIngB;
...
//System.setIn(new FileInputStream("res/input.txt"));
		...
		for (int test_case = 1; test_case <= T; test_case++) {
			...
			for (int i = 0; i < m; i++) {
				...
				badIngA[i] = Integer.parseInt(st.nextToken());
				badIngB[i] = Integer.parseInt(st.nextToken());
			}
	...
	public static boolean check() {
		for (int i = 0; i < m; i++) {
			if (burger.contains(badIngA[i]) && burger.contains(badIngB[i]))
				return false;
		}
